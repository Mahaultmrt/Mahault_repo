...
Res_model <- function(t, pop, param,vec_virus_v) {
  ...
  with(as.list(c(pop, param)), {
    ...
    N=Si+Sa+CRa+CSa+IRa+ISa+S+CR+CS+IR+IS

    infection<- vec_virus_v(t)


    dSi <- -Si*infection*0.5-Si*(1-infection*0.5)

    dSa <- -Sa*((beta*ct*(CRa+IRa+CR+IR)/N)+beta*(CSa+ISa+CS+IS)/N)+delta*IRa+delta*ISa-omega*Sa+teta*S+gamma*CRa+(gamma+alpha)*CSa+Si*infection*atb
    ...
    dS <- -S*((beta*ct*(CRa+IRa+CR+IR)/N)+beta*(CSa+ISa+CS+IS)/N)+delta*IR+delta*IS+omega*Sa-teta*S+gamma*CR+gamma*CS+Si*(1-infection*atb)
...
create_params<-function(beta=1,ct=0.8,delta=0.14,gamma=0.03,rho=0.1,rhoRa=0.08,rhoSa=0,teta=0.022,omega=0.07,alpha=0.5,atb=0.5)
{
  list(beta=beta,ct=ct,delta=delta,gamma=gamma,rho=rho,rhoRa=rhoRa,rhoSa=rhoSa,teta=teta,omega=omega,alpha=alpha,atb=atb)
}

create_initial_cond<-function(Si0=50,Sa0=0,CRa0=1,CSa0=1,IRa0=0,ISa0=0,S0=0,CR0=1,CS0=1,IR0=0,IS0=0){

...
Res_model <- function(t, pop, param,vec_virus) {
  ...
  with(as.list(c(pop, param)), {
    ...
    dSa <- -Sa*((beta*fcost*(CRa+CR)/N)+beta*(CSa+CS)/N)-omega*Sa+new_theta*S+(gamma+alpha*(1-sigmaR))*CRa+(gamma+alpha*(1-sigmaS))*CSa
    dCRa <- Sa*(beta*fcost*(CRa+CR)/N)-(gamma+alpha*(1-sigmaR))*CRa-omega*CRa+new_theta*CR
    ...
    dS <- -S*((beta*fcost*(CRa+CR)/N)+beta*(CSa+CS)/N)+omega*Sa-new_theta*S+gamma*CR+gamma*CS+deltaRa*IRa+deltaSa*ISa
    dCR <- S*(beta*fcost*(CRa+CR)/N)-gamma*CR+omega*CRa-new_theta*CR
...
Res_model_Coli <- function(t, pop, param,vec_virus) {
  ...
  with(as.list(c(pop, param)), {
    ...
    dCSa<- - CSa*(beta*fcost*phi*(CRa+CR)/N)+(gamma+alpha*(1-sigmaR))*CRa-CSa*omega+new_theta*CS
    dCRa<- CSa*(beta*fcost*phi*(CRa+CR)/N)-(gamma+alpha*(1-sigmaR))*CRa-CRa*omega+new_theta*CR
    dCS<- -CS*(beta*fcost*(CRa+CR)/N)+gamma*CR+CSa*omega-new_theta*CS
    dCR<- CS*(beta*fcost*(CRa+CR)/N)-gamma*CR+CRa*omega-new_theta*CR

...
#Code modèle présenté réu 15/05
...
Res_model <- function(t, pop, param,vec_virus) {
  ...
  with(as.list(c(pop, param)), {
    ...
    dIRa <- rhoRa*CRa-delta*IRa-omega*IRa+new_teta*IR
    dISa <- rhoSa*CSa-delta*ISa-omega*ISa+new_teta*IS
    ...
    dIR <- rho*CR-delta*IR+omega*IRa-new_teta*IR
    dIS <- rho*CS-delta*IS+omega*ISa-new_teta*IS
